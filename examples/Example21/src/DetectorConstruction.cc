...
//static std::unordered_map<size_t, size_t> gScoringMap;
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  */
  ...
  // TODO: This defines the size of the HitCollection, we only need one hit per volume of the 
  // ECAL (With the current approach to scoring)
  // Change the size which now makes one hit per volume of the detector
  // fSensitive_volumes is filled through macro commands

  //int numSensitive = vecgeom::GeoManager::Instance().GetPlacedVolumesCount();
  int numSensitive = fSensitive_volumes.size();

  G4cout << "DEBUG: num sensitive: " << numSensitive << G4endl;
  ...
  for (auto name : fSensitive_volumes) {
    ...
    for (auto lvol : store) {
      if (lvol->GetName() == name || lvol->GetName().rfind(name + "0x") == 0)
      {
        //gScoringMap->insert(std::pair<const G4VPhysicalVolume *, int>(g4pvol, pvol->id()));
        SetSensitiveDetector(lvol, caloSD);
      }
    }
  }
