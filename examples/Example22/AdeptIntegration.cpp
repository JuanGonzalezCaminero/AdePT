...
void AdeptIntegration::Shower(int event)
{
  ...
  for(auto pair: (fglobal_vecgeom_to_g4_map))
  {
    ...
    auto geant4_id = pair.second.GET_ID();
    fastSimSensitive->ProcessHits(geant4_id,
                                  fScoring->fScoringPerVolume.energyDeposit[fScoring->fPvolToHit[vecgeom_id]] /
  copcore::units::MeV);
  }
...
void AdeptIntegration::ProcessGPUHits(HostScoring::Stats &aStats)
{
  ...
  G4NavigationHistory *fG4NavigationHistory = new G4NavigationHistory();
  G4Step *fG4Step                           = new G4Step();
  // G4TouchableHistory *fG4TouchableHistory   = new G4TouchableHistory();
  G4TouchableHandle fG4TouchableHistoryHandle = new G4TouchableHistory();
  ...
  for (size_t i = aStats.fBufferStart; i < aStats.fBufferStart + aStats.fUsedSlots; i++) {
    ...
    // Reconstruct G4NavigationHistory
    FillG4NavigationHistory(aNavindex, fG4NavigationHistory);
    ((G4TouchableHistory *)fG4TouchableHistoryHandle())
        ->UpdateYourself(fG4NavigationHistory->GetTopVolume(), fG4NavigationHistory);

    // Reconstruct G4Step
    FillG4Step(&(fScoring->fGPUHitsBuffer_host[aHitIdx]), fG4Step, fG4TouchableHistoryHandle);

    // Call SD code
    SensitiveDetector *aSensitiveDetector =
        (SensitiveDetector *)fG4NavigationHistory->GetVolume(fG4NavigationHistory->GetDepth())
            ->GetLogicalVolume()
    ...
    aSensitiveDetector->ProcessHits(fG4Step, (G4TouchableHistory *)fG4TouchableHistoryHandle());
  }

  delete fG4NavigationHistory;
  delete fG4Step;
}
...
void AdeptIntegration::FillG4Step(GPUHit *aGPUHit, G4Step *aG4Step, G4TouchableHandle &aG4TouchableHandle)
{
  // These need to be allocated as new because:
  // - It gets passed to the track and pre/post step points as a Touchable Handle
  // - If it's allocated on the stack, by the time we set a new Touchable Handle for these, this one has
  // already been freed
  // - When setting a new Touchable Handle G4 tries to release the last one, which leads to a crash
  ...
  aPreStepPoint->SetTouchableHandle(aG4TouchableHandle); // Real data
  ...
  // aPostStepPoint->SetTouchableHandle(aPostStepTouchableHistory);                                   // Missing data
