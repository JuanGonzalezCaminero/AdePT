...
__device__ void HostScoring::RecordHit(char aParticleType, double aStepLength,
...
__device__ __forceinline__ void HostScoring::Copy3DVector(vecgeom::Vector3D<Precision> *source, vecgeom::Vector3D<Precision> *destination)
{
...
bool HostScoring::CheckAndFlush(Stats &aStats_host, HostScoring *aScoring_device, cudaStream_t stream)
{
  ...
  //G4cout << "Info: Hits buffer usage: " << 100 * aBufferUsage << "%" << G4endl;

  if (aBufferUsage > fFlushLimit) {
    // G4cout << "Info: Flushing GPU Hits buffer" << G4endl;
    // G4cout << "Info: Steps since last flush: " << fStepsSinceLastFlush << G4endl;
...
void HostScoring::CopyHitsToHost(Stats &aStats_host, HostScoring *aScoring_device, cudaStream_t stream)
{
  ...
  COPCORE_CUDA_CHECK(cudaMemcpyAsync(fGPUHitsBuffer_host, fGPUHitsBuffer_device,
                                     fBufferCapacity * sizeof(GPUHit), cudaMemcpyDeviceToHost, stream));
  ...
  // Update the used slots counter after all data has been copied,
  // taking into account the slots filled in the meantime
