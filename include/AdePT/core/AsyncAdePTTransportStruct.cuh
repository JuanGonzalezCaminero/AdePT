...
struct GPUstate {
  ...
  ~GPUstate()
  {
    ...
    for (SpeciesState &particleType : particles) {
      if (particleType.stream) ADEPT_DEVICE_API_CALL(cudaStreamDestroy(particleType.stream));
      if (particleType.event) ADEPT_DEVICE_API_CALL(cudaEventDestroy(particleType.event));
    }
