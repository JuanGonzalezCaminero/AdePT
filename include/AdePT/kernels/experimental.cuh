...
__global__ void P1(adept::TrackManager<Track> *gammas, VolAuxData const *auxDataArray)
{
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < activeSize; i += blockDim.x * gridDim.x) {
    ...
    VolAuxData const &auxData = auxDataArray[currentTrack.navState.Top()->GetLogicalVolume()->id()];
...
__global__ void T1(adept::TrackManager<Track> *gammas, MParrayTracks *leakedQueue, VolAuxData const *auxDataArray)
{
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < activeSize; i += blockDim.x * gridDim.x) {
    ...
    double geometryStepLength = BVHNavigator::ComputeStepAndNextVolume(currentTrack.pos, currentTrack.dir,
    ...
    // Store the next state (Is it needed? Used for scoring in kernel P2)
...
__global__ void P2(adept::TrackManager<Track> *gammas, Secondaries secondaries, Scoring *userScoring,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < activeSize; i += blockDim.x * gridDim.x) {
    ...
    case 2: {
      ...
      const double bindingEnergy = G4HepEmGammaInteractionPhotoelectric::SelectElementBindingEnergy(
          &g4HepEmData, auxData.fMCIndex, currentTrack.fPEmxSec, currentTrack.eKin, &rnge);
