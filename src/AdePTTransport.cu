...
G4HepEmState *AdePTTransport::fg4hepem_state{nullptr};
int AdePTTransport::kCapacity          = 1024 * 1024;
int AdePTTransport::kHitBufferCapacity = 1024 * 1024;

void AdePTTransport::VolAuxArray::InitializeOnGPU()
{
...
void AdePTTransport::VolAuxArray::FreeGPU()
{
...
void AdePTTransport::PrepareLeakedBuffers(int numLeaked)
{
...
void AdePTTransport::InitializeGPU()
{
  using TrackData    = adeptint::TrackData;
...
void AdePTTransport::FreeGPU()
{
  ...
  // Free G4HepEm data
  FreeG4HepEmData(AdePTTransport::fg4hepem_state->fData);
  delete AdePTTransport::fg4hepem_state;
  AdePTTransport::fg4hepem_state = nullptr;
}

void AdePTTransport::ShowerGPU(int event, TrackBuffer &buffer) // const &buffer)
{
  using TrackData = adeptint::TrackData;
  ...
  InitTracks<<<initBlocks, initThreads, 0, gpuState.stream>>>(gpuState.toDevice_dev, buffer.toDevice.size(),
                                                              ...
                                                              fScoring_dev, VolAuxArray::GetInstance().fAuxData_dev);
  ...
  do {
    ...
    FinishIteration<<<1, 1, 0, gpuState.stream>>>(gpuState.allmgr_d, gpuState.stats_dev, fScoring_dev);
    COPCORE_CUDA_CHECK(
  ...
  fScoring->CopyHitsToHost(gpuState.stats->hostscoring_stats, fScoring_dev, gpuState.stream);
  // Transfer back the global counters
  // fScoring->fGlobalCounters_dev->numKilled = inFlight;
