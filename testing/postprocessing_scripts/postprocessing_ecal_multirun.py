...
if len(sys.argv) < 5:
	print("Usage: python3 postprocessing_ecal_proportion_multirun.py output_file x_labels_file write_index data_directory [data.csv data_2.csv ...]")
	exit()
...
for dir in os.listdir(data_directory):
    ...
    for i in range(len(data_files)):
        ...
        ecal_mean = np.average(data["ECAL"])
        event_mean = np.average(data["Event"])
        partial_output[x_labels[i]] = [ecal_mean/event_mean]
