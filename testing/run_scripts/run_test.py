...
def run_test(random_seed, configuration_files):
    for config_filename in configuration_files:
        try:
        ...
        for test_id in range(len(config["tests"])):
            ...
            for run_id in range(len(test["runs"])):
                ...
                #If we want to specify the random seed:
                if random_seed > 0:
                    run["configuration"]["random_seed"] = random_seed
                else:
                    run["configuration"]["random_seed"] = 1

                #Add a line to activate or deactivate AdePT (Examples 14 and 17 only)
                if run["executable"] == "example17" or run["executable"] == "example14":
                    if(run["use_adept"]):
                        run["configuration"]["use_adept"] = "/param/ActivateModel AdePT"
                    else:
                        run["configuration"]["use_adept"] = "/param/InActivateModel AdePT"
                else:
                        run["configuration"]["use_adept"] = ""
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python3 run_test.py random_seed configuration_file [configuration_file_2 ...]")
        exit()

    random_seed = int(sys.argv[1])
    configuration_files = sys.argv[2:]

    run_test(random_seed, configuration_files)
